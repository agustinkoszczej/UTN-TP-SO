...
HeapMetadata* read_HeapMetadata(char* page, int offset) {
	...
	int i;
	char* aux = string_new();
	heap_metadata->isFree = page[0];
	for (i = offset; page[i + 1] != '#'; i++) {
		aux[i] = page[i + 1];
	}
...
int write_HeapMetadata(HeapMetadata* heap_metadata, int offset, char* page) {
	int i, j = 0;
	int isFree = heap_metadata->isFree;
	page[offset] = string_itoa(isFree)[0];
	char* aux = string_new();
	aux = string_itoa(heap_metadata->size);
...
bool it_fits_malloc(char* full_page, int size) {
	int offset = 0;
	HeapMetadata* heap_metadata;
	heap_metadata = read_HeapMetadata(full_page, offset);

	if (heap_metadata->isFree && heap_metadata->size <= size){
		heap_metadata->isFree = false;
		write_HeapMetadata(heap_metadata, offset, full_page);
		return true;
	}
	else{
		offset = heap_metadata->size + heap_metadata_size;
	}
	return false;
}
...
void malloc_memory(int pid, int size) { // TODO Work in progress, xdxd
	...
	if (heap_manage->heap_c == 0) {
		...
		char* heap_metadata = string_new();
		string_append(&heap_metadata, "0");
		string_append(&heap_metadata, string_itoa(mem_page_size - heap_metadata_size));
		runFunction(mem_socket, "i_store_bytes_in_page", 5, pid, page, 0,
				string_itoa(heap_metadata_size), heap_metadata);
		wait_response(mem_response);
		heap_manage->heap_c++;
	}
